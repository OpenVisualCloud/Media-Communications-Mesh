...
TEST_F(RdmaTxTest, RdmaTx_SendData)
{
    libfabric_ctx mock_dev_handle; // Mocked RDMA device handle
    char mock_buffer[sizeof(DUMMY_DATA2) - 1] = {};
    std::memcpy(mock_buffer, DUMMY_DATA2, sizeof(DUMMY_DATA2) - 1);
    ...
    // Mock ep_send_buf for repeated calls with enhanced validation
    EXPECT_CALL(*mock_ep_ops, ep_send_buf(::testing::_, ::testing::_, ::testing::_))
        .WillRepeatedly([&](ep_ctx_t *ep_ctx, void *buf, size_t buf_size) -> int {
            // Validate buffer size
            EXPECT_EQ(buf_size, sizeof(DUMMY_DATA2) - 1) << "Buffer size mismatch";
            
            // Validate buffer content
            if (std::memcmp(buf, DUMMY_DATA2, sizeof(DUMMY_DATA2) - 1) != 0) {
                ADD_FAILURE() << "Buffer content does not match expected data.";
            }
    ...
    EXPECT_CALL(*mock_ep_ops, ep_cq_read(::testing::_, ::testing::_, RDMA_DEFAULT_TIMEOUT))
        .WillRepeatedly([&](ep_ctx_t *ep_ctx, void **buf_ctx, int timeout) -> int {
            *buf_ctx = mock_buffer; // Simulate acknowledgment for the sent buffer
            return 0;               // Success
        });
    ...
    // Setup EmulatedTransmitter
    auto emulated_tx = new EmulatedTransmitter(ctx);
    auto res = emulated_tx->establish(ctx);
    ASSERT_EQ(res, connection::Result::success);
    ASSERT_EQ(emulated_tx->state(), connection::State::active);

    // Configure and establish the connection
    ConfigureRdmaTx(conn_tx, ctx, sizeof(mock_buffer));

    // Establish the RdmaTx connection
    res = conn_tx->establish(ctx);
    ASSERT_EQ(res, connection::Result::success);
    ASSERT_EQ(conn_tx->state(), connection::State::active);

    // Link RdmaTx to EmulatedTransmitter
    res = emulated_tx->set_link(ctx, conn_tx);
    ASSERT_EQ(res, connection::Result::success);

    // Transmit data multiple times
    for (int i = 0; i < 5; ++i) {
        res = emulated_tx->transmit_wrapper(ctx, mock_buffer, sizeof(mock_buffer));
        ASSERT_EQ(res, connection::Result::success);
    }
