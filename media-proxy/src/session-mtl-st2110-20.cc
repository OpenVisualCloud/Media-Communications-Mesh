...
rx_st20_mtl_session::rx_st20_mtl_session(mtl_handle dev_handle, struct st20p_rx_ops *opts,
    ...
    : handle(0), frame_thread_handle(0), shm_bufs(0), fb_recv(0)
{
    ...
    if (st_frame_fmt_equal_transport(ops_rx.output_fmt, ops_rx.transport_fmt)) {
        ...
        cleanup();
        throw invalid_argument("output_fmt and transport_fmt differ");
    }
#endif

    handle = st20p_rx_create(dev_handle, &ops_rx);
    if (!handle) {
        cleanup();
        throw runtime_error("Failed to create MTL RX ST20 session");
    }

    frame_size = st20p_rx_frame_size(handle);
    ...
    frame_thread_handle = new std::thread(&rx_st20_mtl_session::frame_thread, this);
...
rx_st20_mtl_session::~rx_st20_mtl_session()
{
    INFO("%s, fb_recv %d\n", __func__, fb_recv);
    cleanup();
}

void rx_st20_mtl_session::consume_frame(struct st_frame *frame)
{
    int err = 0;
    uint16_t qid = 0;
...
tx_st20_mtl_session::tx_st20_mtl_session(mtl_handle dev_handle, struct st20p_tx_ops *opts,
                                         memif_ops_t *memif_ops)
    : handle(0), fb_send(0)
{
    if (!dev_handle || !opts || !memif_ops) {
    ...
    handle = st20p_tx_create(dev_handle, &ops_tx);
    if (!handle) {
        cleanup();
        throw runtime_error("Failed to create MTL TX ST20 session");
    }

    frame_size = st20p_tx_frame_size(handle);
...
void tx_st20_mtl_session::cleanup()
{
    if (handle) {
...
tx_st20_mtl_session::~tx_st20_mtl_session()
{
    INFO("%s, fb_send %d\n", __func__, fb_send);
    cleanup();
}
 ...
 * connection */
#if defined(MTL_ZERO_COPY)
int rx_st20_mtl_session::query_ext_frame_cb(struct st_ext_frame *ext_frame,
                                            struct st20_rx_frame_meta *meta)
{
    int err = 0;
    ...
    for (uint8_t plane = 0; plane < planes; plane++) { /* assume planes continuous */
        ext_frame->linesize[plane] = st_frame_least_linesize(output_fmt, meta->width, plane);
        if (plane == 0) {
...
int rx_st20_mtl_session::on_connect_cb(memif_conn_handle_t conn)
{
 ...
 * identify connection */
int rx_st20_mtl_session::on_disconnect_cb(memif_conn_handle_t conn)
{
    if (atomic_load_explicit(&shm_ready, memory_order_relaxed) &&
...
int tx_st20_mtl_session::frame_done_cb(struct st_frame *frame)
{
    uint16_t qid = 0;
    uint16_t buf_num = 1;
...
int tx_st20_mtl_session::on_connect_cb(memif_conn_handle_t conn)
{
    memif_region_details_t region;
...
/* informs user about disconnected status. private_ctx is used by user to
 * identify connection */
int tx_st20_mtl_session::on_disconnect_cb(memif_conn_handle_t conn)
{
    if (atomic_load_explicit(&shm_ready, memory_order_relaxed) &&
        mtl_dma_unmap(st, source_begin, source_begin_iova, source_begin_iova_map_sz) < 0) {
        ERROR("Fail to unmap DMA memory address.");
...
int tx_st20_mtl_session::on_receive_cb(memif_conn_handle_t conn, uint16_t qid)
{
