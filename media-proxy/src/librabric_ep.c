...
static int ep_av_insert(libfabric_ctx* rdma_ctx, struct fid_av *av, void *addr, size_t count, fi_addr_t *fi_addr,
...
int ep_init_av_addr(ep_ctx_t* ep_ctx, libfabric_ctx* rdma_ctx, struct fi_info* fi)
{
    ...
    if (fi->dest_addr) {
        ...
        ret = ep_av_insert(rdma_ctx, ep_ctx->av, ep_ctx->data_buf,
                   1, &ep_ctx->dest_av_entry, 0, NULL);
        if (ret)
            return ret;
    }
...
static int ep_alloc_res(ep_ctx_t* ep_ctx, libfabric_ctx* rdma_ctx,
                        struct fi_info* fi, size_t tx_cq_size,
                        size_t rx_cq_size, size_t av_size)
{
    int ret;
	...
	if (!ep_ctx->av && (rdma_ctx->info->ep_attr->type == FI_EP_RDM || rdma_ctx->info->ep_attr->type == FI_EP_DGRAM)) {
...
static int ep_reg_mr(ep_ctx_t* ep_ctx, libfabric_ctx* rdma_ctx, struct fi_info* fi) {
    ...
    ret = rdma_reg_mr(rdma_ctx, ep_ctx->ep, fi, ep_ctx->data_buf, ep_ctx->data_buf_size,
            rdma_info_to_mr_access(fi), (uint64_t)ep_ctx, FI_HMEM_SYSTEM,
            0, &ep_ctx->data_mr, &ep_ctx->data_desc);
    if (ret)
        return ret;
...
int ep_send_buf(ep_ctx_t* ep_ctx, char* buf, size_t buf_size)
{
    ...
    do {
        ret = fi_send(ep_ctx->ep, buf, buf_size, ep_ctx->data_desc,
                      ep_ctx->dest_av_entry, ep_ctx->send_ctx);
        if (ret == -FI_EAGAIN)
...
int ep_recv_buf(ep_ctx_t *ep_ctx, char* buf, size_t buf_size)
{
    ...
    do {
        ret = fi_recv(ep_ctx->ep, buf, buf_size, ep_ctx->data_desc,
                      FI_ADDR_UNSPEC, ep_ctx->recv_ctx);
        if (ret == -FI_EAGAIN)
...
int ep_init(ep_ctx_t** ep_ctx, ep_cfg_t* cfg) {
    ...
    *ep_ctx = calloc(1, sizeof(ep_ctx_t));
    ...
    if(cfg->dir == RX) {
        ret = fi_getinfo(FI_VERSION(1,21), NULL, cfg->local_addr.port,
                         FI_SOURCE, hints, &fi);
    } else {
        ret = fi_getinfo(FI_VERSION(1,21), cfg->remote_addr.ip, cfg->remote_addr.port,
                    0, hints, &fi);
    }
    if (ret) {
