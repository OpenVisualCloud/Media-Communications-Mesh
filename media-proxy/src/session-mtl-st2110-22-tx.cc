...
static int tx_frame_done_callback_wrapper(void *priv, struct st_frame *frame)
{
    ...
    TxSt22MtlSession *s = (TxSt22MtlSession *)priv;
    return s->frame_done_cb(frame);
}

int TxSt22MtlSession::frame_done_cb(struct st_frame *frame)
{
    uint16_t qid = 0;
    uint16_t buf_num = 1;
...
int TxSt22MtlSession::on_connect_cb(memif_conn_handle_t conn)
{
...
int TxSt22MtlSession::on_disconnect_cb(memif_conn_handle_t conn)
{
...
void TxSt22MtlSession::copy_connection_params(const mcm_conn_param &request, std::string dev_port)
{
    ...
    snprintf(session_name, NAME_MAX, "mcm_tx_st22_%d", get_id());
...
TxSt22MtlSession::TxSt22MtlSession(mtl_handle dev_handle, const mcm_conn_param &request,
...
int TxSt22MtlSession::init()
{
    handle = st22p_tx_create(st, &ops);
    if (!handle) {
        ERROR("Failed to create MTL TX ST22 session");
        return -1;
    }
...
TxSt22MtlSession::~TxSt22MtlSession()
{
    INFO("%s, fb_send %d\n", __func__, fb_send);
    stop = true;
    if (handle) {
...
int TxSt22MtlSession::on_receive_cb(memif_conn_handle_t conn, uint16_t qid)
{
