...
void usage(FILE* fp, const char* path)
{
    ...
    fprintf(fp, "-s, --fn=file_name\t\t\t"
                "Save stream to local file (default: %s).\n", DEFAULT_LOCAL_FILE);
...
int main(int argc, char** argv)
{
    ...
    char file_name[128] = DEFAULT_LOCAL_FILE;
    ...
    default:
        ...
        param.payload_args.video_args.pix_fmt = PIX_FMT_NV12;
        break;
    }
    ...
    while (keepRunning) {
        ...
        if (strncmp(payload_type, "rtsp", sizeof(payload_type)) != 0) {
            ...
            // usleep(300000);
            size_t len = *(size_t*)(ptr + sizeof(buf->metadata.seq_num) + sizeof(buf->metadata.timestamp));
            void* data = ptr + sizeof(buf->metadata.seq_num) + sizeof(buf->metadata.timestamp) +sizeof(buf->len);
            ...
            printf("RX package number:%d   RX H264 NALU: %ld\n", frame_count, len);
        }
    }
