...
mcm_buffer* memif_dequeue_buffer(mcm_conn_context* conn_ctx, int timeout, int* error_code)
{
    ...
    if (conn_ctx->type == is_tx) {
        ...
        if (err == MEMIF_ERR_SUCCESS) {
            ...
            buf->len = memif_buf.len - sizeof(buf->metadata) - sizeof(buf->len);
            buf->data = memif_buf.data + sizeof(buf->metadata) + sizeof(buf->len);
        ...
        if (memif_conn->buf_num > 0) {
            ...
            buf->len = memif_conn->working_bufs[memif_conn->working_idx].len - sizeof(buf->metadata) - sizeof(buf->len);
            buf->data = memif_conn->working_bufs[memif_conn->working_idx].data + sizeof(buf->metadata) + sizeof(buf->len);
...
int memif_enqueue_buffer(mcm_conn_context* conn_ctx, mcm_buffer* buf)
{
    ...
    if (conn_ctx->type == is_tx) {
        if (buf->data != memif_conn->working_bufs[0].data + sizeof(buf->metadata) + sizeof(buf->len)) {
