...
# TODO: Edit below dictionaries, so JSON files can be used, but leave defaults
ip_dict = dict(
    rx_interfaces="192.168.96.2",
    tx_interfaces="192.168.96.3",
    rx_sessions="192.168.85.20",
    tx_sessions="192.168.85.20",
)

ip_dict_rgb24_multiple = dict(
    p_sip_1="192.168.96.2",
    p_sip_2="192.168.96.3",
    p_tx_ip_1="192.168.108.202",
    p_tx_ip_2="192.168.108.203",
)
...
def execute_test(
    ...
    if not multiple_sessions:
        ...
        if tx_is_ffmpeg:
            ...
            tx_cmd = f"./build/app/RxTxApp --config_file {tx_config_file}"
...
def execute_test_rgb24(
    ...
    rx_cmd = f"./build/app/RxTxApp --config_file {rx_config_file}"
...
def execute_test_rgb24_multiple(
    ...
    rx_config_file = generate_rxtxapp_rx_config_rgb24_multiple(nic_port_list[:2], type_, video_format_list, pg_format, True)
    rx_cmd = f"./build/app/RxTxApp --config_file {rx_config_file}"
