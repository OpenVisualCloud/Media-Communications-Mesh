...
void print_usage(const char *prog_name) {
    ...
    printf("SMPTE ST 2110 Receiver Test Application\n\n");
    printf("Options:\n");
    ...
    printf("\n  Output options:\n");
    printf("  -o, --output <file>        Save received data to file\n");
    printf("\n  General:\n");
    printf("  -h, --help                 Show this help\n");
    printf("  -n, --frames <count>       Number of frames/packets to receive (-1 for infinite, default: 100)\n");
}
...
int parse_arguments(int argc, char **argv) {
    ...
    int c;
    while ((c = getopt_long(argc, argv, "t:i:p:s:P:W:H:f:c:r:q:d:o:n:h", long_options, NULL)) != -1) {
        switch (c) {
            case 't':
            ...
            case 'd':
                ...
                break;
            case 'o':
                strncpy(st2110_cfg.output_file, optarg, sizeof(st2110_cfg.output_file) - 1);
                st2110_cfg.save_output = 1;
                break;
            case 'n':
...
int main(int argc, char **argv) {
    ...
    input_loop = 100; // Default to receiving 100 frames/packets
    parse_arguments(argc, argv);

    LOG("[RX] Launching ST 2110 RX app with transport: %s", st2110_cfg.transport);
    LOG("[RX] Listening on: %s:%d, Payload Type: %d", st2110_cfg.ip_addr, st2110_cfg.port, st2110_cfg.payload_type);
    
    if (strcmp(st2110_cfg.transport, "st2110-30") == 0) {
        LOG("[RX] Audio: %d channels, %d Hz, %s, %s", 
            st2110_cfg.channels, st2110_cfg.sample_rate, 
            st2110_cfg.audio_format, st2110_cfg.packet_time);
    } else {
        LOG("[RX] Video: %dx%d @ %.1f fps, %s", 
            st2110_cfg.width, st2110_cfg.height, st2110_cfg.fps, st2110_cfg.pixel_format);
    }
    LOG("[RX] RDMA: provider=%s, endpoints=%d", st2110_cfg.rdma_provider, st2110_cfg.rdma_num_endpoints);
    
    if (st2110_cfg.save_output) {
        LOG("[RX] Saving output to: %s", st2110_cfg.output_file);
    }
    ...
    /* Receive data based on payload type */
    if (strcmp(st2110_cfg.transport, "st2110-30") == 0) {
        // Audio reception
        if (input_loop == -1) {
            LOG("[RX] receiving audio packets inf times");
            while (1) {
                err = mcm_recv_audio_packets(connection, st2110_cfg.save_output ? st2110_cfg.output_file : NULL);
                if (err) {
                    LOG("[RX] Failed to receive audio packets: %s (%d)", mesh_err2str(err), err);
                    break;
                }
            ...
            LOG("[RX] receiving %d audio packets", input_loop);
            ...
            LOG("[RX] receiving 1 audio packet");
            err = mcm_recv_audio_packets(connection, st2110_cfg.save_output ? st2110_cfg.output_file : NULL);
            if (err) {
                LOG("[RX] Failed to receive audio packets: %s (%d)", mesh_err2str(err), err);
            }
        ...
        // Video reception (st2110-20 or st2110-22)
        if (input_loop == -1) {
            LOG("[RX] receiving video frames inf times");
            while (1) {
                err = mcm_recv_video_frames(connection, st2110_cfg.save_output ? st2110_cfg.output_file : NULL);
                if (err) {
                    LOG("[RX] Failed to receive video frames: %s (%d)", mesh_err2str(err), err);
                    break;
                }
                if (shutdown_flag == SHUTDOWN_REQUESTED) {
                    break;
            ...
            LOG("[RX] receiving %d video frames", input_loop);
            for (int i = 0; i < input_loop; i++) {
                err = mcm_recv_video_frames(connection, st2110_cfg.save_output ? st2110_cfg.output_file : NULL);
                if (err) {
                    LOG("[RX] Failed to receive video frames: %s (%d)", mesh_err2str(err), err);
            ...
            LOG("[RX] receiving 1 video frame");
            err = mcm_recv_video_frames(connection, st2110_cfg.save_output ? st2110_cfg.output_file : NULL);
            if (err) {
                LOG("[RX] Failed to receive video frames: %s (%d)", mesh_err2str(err), err);
            }
