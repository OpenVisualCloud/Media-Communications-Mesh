...
func (lc *logicController) decodeActionsRecursive(content []*yaml.Node) ([]manifestAction, error) {
	...
	for i, node := range content {
		...
		n := strings.Index(actionName, "(")
		if n >= 0 && strings.HasSuffix(actionName, ")") {
			modifier = actionName[n+1 : len(actionName)-1]
			actionName = actionName[:n]
		}

		action, ok := lc.definition.actions[actionName]
		if !ok {
			return nil, fmt.Errorf("unknown action name (%v)", actionName)
		}

		if len(modifier) > 0 {
			err := action.ValidateModifier(modifier)
			if err != nil {
				return nil, fmt.Errorf("validate modifier failed [%v]: %v", node.Value, err)
			}
		}
		...
		if err != nil {
			return nil, fmt.Errorf("action decode err: %w", err)
		}
...
func (lc *logicController) PrintManifestJson() {
	...
	if err != nil {
		logrus.Errorf("JSON error %v\n", err)
	} else {
		logrus.Debugf("JSON business logic manifest\n%v\n", string(out))
	}
}
