...
func (a *API) ListMultipointGroups(w http.ResponseWriter, r *http.Request) {
	...
	groups, err := registry.MultipointGroupRegistry.List(r.Context(), nil, addStatus, addConfig)
	if err != nil {
	...
	resp := struct {
		MultipointGroups []model.MultipointGroup `json:"multipointGroup"`
	}{
		MultipointGroups: groups,
	}
